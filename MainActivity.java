    ...
    public class AppBridge {
        ...
        public void openIndividualRow(String excelRowNum, String displayKey) {
            ...
            new Thread(() -> {
                try {
                    ...
                    String msg = result.asList().get(1).toString();
                    if (!ok) {
                        runOnUiThread(() -> showOpenError("Generate failed for " + key + ":\n" + msg));
                        return;
                    }
                    String safeKey = key.replaceAll("[^a-zA-Z0-9._-]+", "_");
                    if (safeKey.length() > 40) safeKey = safeKey.substring(0, 40);
                    ...
                    runOnUiThread(() -> showOpenError(
                            "Open individual failed: " + e.getMessage()));
                }
            }).start();
        }
        ...
        public void openAnalysedFile(String path) {
            ...
            runOnUiThread(() -> {
                ...
                openDocumentFlexible(p.trim());
            });
        }
                try {
                    MainActivity.this.openDocumentFlexible(path.trim());
                } catch (Exception e) {
                    Toast.makeText(MainActivity.this,
