...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public void onPageFinished(WebView view, String url) {
                // Prevent Back from cycling Generator ↔ Analyser ↔ splash
                view.clearHistory();
            }
            @Override
            public void onReceivedError(WebView view, int errorCode, String description, String failingUrl) {
                if (failingUrl != null && failingUrl.contains("splash.html")) {
    ...
    /** Open any local path or content URI. Never throws to WebView bridge. */
    private void openDocumentFlexible(String pathOrUri) {
        if (pathOrUri == null || pathOrUri.trim().isEmpty()) {
            showOpenError("Empty file path");
            return;
        }
        ...
        try {
            ...
            if (pathOrUri.startsWith("content://")) {
                ...
                if (mime == null || mime.isEmpty()) {
                    mime = mimeForName(pathOrUri);
                }
                if (mime == null || mime.isEmpty()) mime = "*/*";
            } else {
                File file = new File(pathOrUri);
                if (!file.exists()) {
                    showOpenError("File not found on device:\n" + pathOrUri
                            + "\n\nIt may have been cleared from cache. Open the batch from Table 2 or generate again.");
                    return;
                }
                if (!file.canRead()) {
                    showOpenError("File exists but is not readable:\n" + pathOrUri);
                    return;
                }
                try {
                    uri = FileProvider.getUriForFile(this, getPackageName() + ".provider", file);
                } catch (Exception e) {
                    showOpenError("FileProvider failed: " + e.getMessage()
                            + "\nPath: " + pathOrUri);
                    return;
                }
                mime = mimeForName(file.getName());
            }

            Intent intent = new Intent(Intent.ACTION_VIEW);
            intent.setDataAndType(uri, mime);
            intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            // Do NOT use FLAG_ACTIVITY_NEW_TASK from an Activity — can block chooser on some OEMs
            try {
                startActivity(Intent.createChooser(intent, "Open with"));
                Toast.makeText(this, "Opening file…", Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
                showOpenError("No app can open this file.\nMIME: " + mime
                        + "\nError: " + e.getMessage());
            }
        } catch (Exception e) {
            showOpenError("Open failed: " + e.getMessage());
        }
    }
     ...
     * - READ_EXTERNAL_STORAGE (API ≤ 32) for older devices
     * - POST_NOTIFICATIONS (API 33+)
     * No READ_MEDIA_IMAGES/VIDEO (Play requires system picker; we do not browse the gallery)
     * Excel photos are embedded in the .xlsx and need no media permission.
     */
    ...
    public class AppBridge {
        ...
        @JavascriptInterface
        /**
         * Generate one row into app cache only (for batch merge).
         * Does NOT write to Downloads — avoids 50 loose files.
         * Returns OK:cachePath or ERROR:...
         */
        ...
        /**
         * On-demand: build one person's DOCX, save to Downloads, then open.
         * Called from Table 1 Open — only creates a file when the user asks.
         */
        @JavascriptInterface
        public void openIndividualRow(String excelRowNum, String displayKey) {
            final String rowNum = excelRowNum;
            final String key = displayKey != null ? displayKey : ("Row " + excelRowNum);
            runOnUiThread(() -> Toast.makeText(MainActivity.this,
                    "Preparing document for " + key + "…", Toast.LENGTH_SHORT).show());
            new Thread(() -> {
                try {
                    if (cachedExcelPath == null || cachedWordPath == null) {
                        runOnUiThread(() -> showOpenError(
                                "Excel/Word not loaded. Upload both files again, then Open."));
                        return;
                    }
                    ensurePython();
                    File out = new File(getCacheDir(),
                            "open_row_" + rowNum + "_" + System.currentTimeMillis() + ".docx");
                    Python py = Python.getInstance();
                    PyObject gen = py.getModule("generator");
                    PyObject result = gen.callAttr(
                            "generate_one_row",
                    ...
                    String fileName = "DocsPlus_" + safeKey + "_" + rowNum + ".docx";
                    String published = publishToDownloads(out, fileName);
                    try {
                        out.delete();
                    } catch (Exception ignored) {
                    }
                    final String pub = published;
                    final String fn = fileName;
                    runOnUiThread(() -> {
                        Toast.makeText(MainActivity.this,
                                "Saved: " + fn, Toast.LENGTH_SHORT).show();
                        showOpenDialog(pub, fn);
                    });
                } catch (Exception e) {
                    final String em = e.getMessage() != null ? e.getMessage() : "unknown error";
                    runOnUiThread(() -> showOpenError("Open individual failed: " + em));
                }
            }).start();
        }
        ...
        public String mergeAndPublishBatch(String pathsJson) {
            try {
                ...
                // Keep per-row temp files so Table 1 "Open" still works this session
                try { merged.delete(); } catch (Exception ignored) {}
                // Ask user to open the batch file (must run on UI thread)
                showOpenDialog(published, fileName);
                runOnUiThread(() -> Toast.makeText(MainActivity.this,
                        "Batch saved to Downloads: " + fileName, Toast.LENGTH_LONG).show());
        ...
        public void openAnalysedFile(String path) {
            ...
            runOnUiThread(() -> {
                if (p == null || p.trim().isEmpty()) {
                    showOpenError("Cannot open: empty path (nothing was saved for this row)");
                    return;
                }
                MainActivity.this.openDocumentFlexible(p.trim());
            });
        }

        /** Public so JS can force the Yes/No open dialog after generate */
        @JavascriptInterface
        public void offerOpenFile(String path, String displayName) {
            if (path == null || path.isEmpty()) {
                runOnUiThread(() -> Toast.makeText(MainActivity.this,
                        "No file path to open", Toast.LENGTH_LONG).show());
                return;
            }
            showOpenDialog(path, displayName != null ? displayName : "document");
        }
    ...
    /** Master exit: always confirm, never walk WebView history (avoids tab loops). */
    @Override
